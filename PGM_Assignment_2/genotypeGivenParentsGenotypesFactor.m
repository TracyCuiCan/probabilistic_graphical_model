...
for i=1:(prod(genotypeFactor.card))
    IDs=IndexToAssignment(i,genotypeFactor.card);
    child=genotypesToAlleles(IDs(1),:);
    p1=genotypesToAlleles(IDs(2),:);
    p2=genotypesToAlleles(IDs(3),:);
    possible=[p1(1),p2(1);p1(1),p2(2);p1(2),p2(1);p1(2),p2(2);];
    for j=1:4
        if possible(j,2)<possible(j,1)
            temp=possible(j,2);
            possible(j,2)=possible(j,1);
            possible(j,1)=temp;
        end
        if (child(1)==possible(j,1)) && (child(2)==possible(j,2))
